...
| `GET /api/stats/playtimes` | When the servers are busy (the `/stats` page). Finishes bucketed into a 7×24 hour-of-week grid, split by server region. `?days=` ∈ `30`, `90` (default), `365`, `0` (all time); `?tz=` any IANA zone (default `UTC`) — both are snapped to an allow-list, so an unknown value falls back rather than erroring. Returns `{tz, days, first, last, total, regions[], cells[]}`; a `cells` row is `{region, dow, hour, runs, players}` with `dow` 0 = Monday and `dow`/`hour` = `-1` marking a rollup (region `"ALL"` likewise). Rollups are counted, not summed: distinct players do not add up across regions or hours. |
