...
const ABOUT_CMDS = [
  ...
  },
  {
    ...
    rows: [
      ["/reverse", "Turn reverse mode on. The first press drops you into noclip — fly to the map's finish line, which is your reverse start. Press again to leave noclip and stand ready."],
...
const ABOUT_FAQ = [
  ...
  ["Why wasn't my time saved?",
    "Times only count in a clean race-mode run. If you toggled <span class=\"mono\">/practicemode</span>, <span class=\"mono\">/noclip</span>, or used <span class=\"mono\">/position</span>, that run won't be recorded. Use <span class=\"mono\">/kill</span> to get back to the start and race it straight through. (The one exception is reverse mode's setup noclip — see below — which is not practicemode, so a reverse run still counts.)"],
  ["What is reverse mode?",
    "Reverse mode lets you race a map <b>backwards</b>. Type <span class=\"mono\">/reverse</span> to turn it on: you drop into noclip so you can fly to the map's <b>finish</b> line — that's your reverse start. Press <span class=\"mono\">/reverse</span> again to leave noclip and stand ready, then cross the finish line to start the clock, run the checkpoints in reverse, and cross the <b>start</b> line to finish. Prejump rules still apply, and the setup noclip is <b>not</b> practicemode, so the run counts. Reverse times live on their own separate leaderboard — the map appears here as <span class=\"mono\">&lt;map&gt;-reversed</span> with a <span class=\"pill rev\">REVERSE</span> badge — and never mix with the normal times. Use <span class=\"mono\">/reverse off</span> to go back to a normal run."],
