/*
 * Entity finder: an index of notable map entities (start/finish timers, weapons,
 * pushers, doors, buttons, checkpoints, teleporter destinations, slick surfaces)
 * built once at map spawn (see the population loop in GT_SpawnGametype). It backs
 * the "/position find <type>" teleport/map-learning tool and map-stats reporting.
 *
 * Ported verbatim from hettoo/wsw-race@racemod (entityfinder.as); it is
 * self-contained and uses only stock natives (Centre/G_GetEntity/findTargeting).
 * The global instance and the population calls live in hrace.as.
 */
...
class EntityFinder
{
    ...
    EntityList@ allEntities( String type )
    {
        ...
        {
            // Debug fallback: a raw entity number. Reject non-positive values —
            // any non-numeric typo parses to 0 (worldspawn), which would
            // otherwise "find" the map's bounding-box centre and teleport the
            // player into the void. (The engine's G_GetEntity itself bounds-
            // checks and returns null for out-of-range indices.)
            int entNum = type.toInt();
            if ( entNum <= 0 )
                return Nil();
            Entity@ ent = G_GetEntity( entNum );
            if ( @ent == null )
                return Nil();
